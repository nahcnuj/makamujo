...
const GAME_ROW_LABELS = ["現在のゲーム", "ソルバー状態"] as const;
...
const formatSolverStateValue = (solverState: Record<string, unknown> | undefined): string => {
  if (solverState === undefined) {
    return "-";
  }
  return JSON.stringify(solverState, null, 2);
};
...
export const createAgentStatusRows = (stateResponse: AgentStateResponse | null): AgentStatusRow[] => {
  ...
  if (stateResponse !== null && stateResponse !== undefined && "currentGame" in stateResponse) {
    ...
    rows.push({
      label: "ソルバー状態",
      value: formatSolverStateValue(stateResponse.currentGame?.state),
      isPreformatted: true,
    });
  }
