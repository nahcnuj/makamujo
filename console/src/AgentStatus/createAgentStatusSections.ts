...
export const createAgentStatusSections = (
  ...
  const markovModelRows = rows.filter((row) =>
    MARKOV_MODEL_ROW_LABEL_SET.has(row.label),
  );
  const gameRows = rows.filter((row) => GAME_ROW_LABEL_SET.has(row.label));

  const shouldShowGameSection =
    stateResponse !== null && stateResponse !== undefined;
  ...
  const sections = [
    ...
    ...(shouldShowGameSection
      ? [{ title: gameSectionTitle, rows: gameRows }]
      : []),
  ];

  return sections.filter(
    (section) => section.rows.length > 0 || section.title === gameSectionTitle,
  );
};
