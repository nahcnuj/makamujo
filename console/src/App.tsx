...
const buildLayout = (
  ...
  const totalHeight =
    layoutEntries.reduce((acc, entry) => acc + entry.height, 0) +
    ROW_GAP * (layoutEntries.length - 1);
...
const CandidateList = ({ candidates, dist, depth, layout }: CandidateListProps) => {
    ...
    <div className="candidate-row" style={nodeStyle} data-depth={depth}>
      ...
      <div className="candidate-children">
        {canExpand ? (
          ...
          <div className="candidate-truncate">
            {depth + 1 >= MAX_DEPTH ? "depth limit" : "no next"}
          </div>
        )}
      </div>
    </div>
