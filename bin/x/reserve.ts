...
// noVNC is unauthenticated and shows OBS, so it runs only while this script does.
...
const startNoVnc = async (): Promise<void> => {
  try {
    ...
    console.warn(
      "[WARN] failed to start noVNC; interactive login is unavailable:",
...
try {
  ...
  if (!hasNiconicoSession(await readCookies())) {
    console.debug(`Not signed in to Niconico. Signing in via ${NOVNC_URL}...`);
