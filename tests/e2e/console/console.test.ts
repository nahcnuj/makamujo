...
test.describe("console", () => {
  ...
  test("connects via SSE and updates on broadcast (non-mock)", async ({ page }) => {
    ...
    try {
      ...
      try { await consoleEnvRes.json(); } catch {}
    } catch (_err) {
    }
    ...
    try {
      await page.waitForFunction(
        () => (window as any).__sseOpen === true || (window as any).__sseMessageReceived === true,
        { timeout: 60_000 },
      );
    } catch {
      // If __sseOpen/message detection fails after 60s, check if we at least got data flowing
      const hasReceivedData = await page.evaluate(() => (window as any).__sseMessageReceived === true);
      if (!hasReceivedData) {
        throw new Error('SSE connection failed: no __sseOpen signal and no messages received after 60s');
      }
    }
  ...
  test("reloads when broadcasted meta becomes 公開終了", async ({ page }) => {
    ...
    await page.goto(`${CONSOLE_BASE_URL}/console/`, { waitUntil: 'domcontentloaded', timeout: BROWSER_PAGE_LOAD_TIMEOUT_MS });
    await page.waitForFunction(() => (window as any).__sseOpen === true, { timeout: 60_000 });
  ...
  test("keeps SSE connection open while the console browser tab is open", async ({ page }) => {
    ...
    await page.goto(`${CONSOLE_BASE_URL}/console/`, { waitUntil: 'domcontentloaded', timeout: BROWSER_PAGE_LOAD_TIMEOUT_MS });
    await page.waitForFunction(() => (window as any).__sseOpen === true, { timeout: 60_000 });
    await page.waitForFunction(() => (window as any).__sseMessageCount > 0, { timeout: 60_000 });
