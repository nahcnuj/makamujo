...
const makeGetComputedStyle =
  (styles: Map<ElementLike, StyleLike>) =>
...
describe("collectClickableElementIds", () => {
  it("returns an empty array when there are no elements", () => {
    ...
    const result = collectClickableElementIds(
      [],
      boundary,
      makeGetComputedStyle(new Map()),
    );
  ...
  it("includes a visible element with cursor:pointer", () => {
    ...
    const styles = new Map([
      [el, { cursor: "pointer", pointerEvents: "auto" }] as const,
    ]);
    const result = collectClickableElementIds(
      [el],
      boundary,
      makeGetComputedStyle(styles),
    );
  ...
  it("excludes an element with pointer-events:none", () => {
    ...
    const styles = new Map([
      [el, { cursor: "pointer", pointerEvents: "none" }] as const,
    ]);
    const result = collectClickableElementIds(
      [el],
      boundary,
  ...
  it("excludes an element without cursor:pointer", () => {
    ...
    const styles = new Map([
      [el, { cursor: "default", pointerEvents: "auto" }] as const,
    ]);
    const result = collectClickableElementIds(
      [el],
      boundary,
  ...
  it("excludes clickable descendants inside #smallSupport", () => {
    const boundary = makeBoundary();
    const smallSupport = makeElement("smallSupport", {
      parentElement: boundary,
    });
    const child = makeElement("smallSupportChild", {
      parentElement: smallSupport,
    });
    const styles = new Map([
  ...
  it("includes only elements within the prompt boundary when a modal popup is open", () => {
    ...
    const insidePrompt = makeElement("promptClose", {
      parentElement: promptBoundary,
    });
...
describe("buildSightResult", () => {
  ...
  it("reflects isWrinkled from cpsIsWrinkled", () => {
    expect(
      buildSightResult({ ...baseSightRawData, cpsIsWrinkled: false })
        .isWrinkled,
    ).toBe(false);
    expect(
      buildSightResult({ ...baseSightRawData, cpsIsWrinkled: true }).isWrinkled,
    ).toBe(true);
  });
  ...
  it("sets bulkMode to buy when storeBulkBuy is selected", () => {
    const result = buildSightResult({
      ...baseSightRawData,
      storeBulkModeSelectedId: "storeBulkBuy",
    });
  ...
  it("sets bulkMode to sell when storeBulkSell is selected", () => {
    const result = buildSightResult({
      ...baseSightRawData,
      storeBulkModeSelectedId: "storeBulkSell",
    });
