...
describe("solver", () => {
  ...
  it("seeStats opens the stats menu then sights", () => {
    ...
    expect(solve.next(ActionResult.ok(stats) as any).value).toEqual(
      Action.noop,
    );
  });
  ...
  it("save exports save data then dismisses the dialog", () => {
    ...
    expect(solve.next(ActionResult.ok(options) as any).value).toEqual(
      exportSave,
    );
    expect(solve.next(ActionResult.ok(exportSave) as any).value).toEqual(
      Action.noop,
    );
    expect(
      solve.next({
...
describe("handleSeeStats", () => {
  it("opens stats, sights, then returns idle", () => {
    const gen = handleSeeStats(
      { type: "seeStats", phase: "open", failureCount: 0 },
      baseCtx(),
    );
...
describe("handleSave", () => {
  it("exports save, notifies onSave, then enters seeStats", () => {
    ...
    const gen = handleSave(
      { type: "save", phase: "options", failureCount: 0 },
      ctx,
    );
    const options = Action.clickByText("オプション");
    const exportSave = Action.clickByText("セーブをエクスポート");
...
describe("step* remaining branches", () => {
  ...
  it("hydrate fills default phases", () => {
    expect(hydrate({ type: "initialize" } as any)).toMatchObject({
      phase: "open",
    });
    expect(hydrate({ type: "idle", count: 2 })).toMatchObject({
      phase: "sight",
      count: 2,
    });
    expect(hydrate({ type: "save", failureCount: 1 })).toMatchObject({
      phase: "options",
    });
    expect(hydrate({ type: "seeStats", failureCount: 0 })).toMatchObject({
      phase: "open",
    });
...
describe("States factories and thin handles", () => {
  it("States helpers return hydrated shapes", () => {
    expect(States.initialize("D")).toMatchObject({
      type: "initialize",
      phase: "open",
      data: "D",
    });
    expect(States.idle(5)).toMatchObject({
      type: "idle",
      phase: "sight",
      count: 5,
    });
    expect(States.save(2)).toMatchObject({
      type: "save",
      phase: "options",
      failureCount: 2,
    });
    expect(States.seeStats(1)).toMatchObject({
      type: "seeStats",
      phase: "open",
      failureCount: 1,
    });
...
describe("obvious step* branches", () => {
  ...
  it("stepInitialize closed event", () => {
    expect(
      stepInitialize(States.initialize(), { name: "closed" } as any).state,
    ).toEqual({ type: "closed" });
  });

  it("stepSave closed event", () => {
    expect(
      stepSave(States.save(), { name: "closed" } as any, saveCtx).state,
    ).toEqual({ type: "closed" });
  });
  ...
  it("stepInitialize open failure stays on open", () => {
    ...
    const out = stepInitialize(
      States.initialize(),
      ActionResult.error(open) as any,
    );
  ...
  it("runActions closed on Escape after click failure", () => {
    ...
    expect(solve.next(ActionResult.error(click as any) as any).value).toEqual({
      name: "press",
      key: "Escape",
    });
