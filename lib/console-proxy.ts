...
export async function fetchMetaSnapshot(proxyBase: string): Promise<any> {
  try {
    ...
    try {
      ...
      try {
        console.warn(
          "[DIAG] fetchMetaSnapshot final fetch failed",
          String(err),
        );
      } catch {}
      return {};
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      try {
        console.debug &&
          console.debug(
            "[DIAG] resilient proxy enqueued initial :connected chunk",
          );
      } catch {}
      ...
      (async () => {
        try {
          ...
          try {
            ...
            try {
              console.error(
                "[DIAG] resilient proxy initial body read failed",
                String(err),
              );
            } catch {}
          ...
          try {
            console.log(
              "[DIAG] resilient proxy initial upstream body completed",
            );
          } catch {}
          ...
          while (!stopped) {
            ...
            try {
              ...
              try {
                ...
                try {
                  console.error(
                    "[DIAG] resilient proxy reconnect body read failed",
                    String(err),
                  );
                } catch {}
              ...
              try {
                console.warn(
                  "[DIAG] resilient proxy reconnect fetch failed",
                  String(err),
                );
              } catch {}
      ...
      if (isSelfProxy) {
        ...
        const stream = new ReadableStream<Uint8Array>({
          start(controller) {
            ...
            try {
              const immediate =
                (globalThis as any).__getCurrentStreamPayload?.() ??
            ...
            const pollStopTimer = setTimeout(() => {
              ...
              try {
                (controller as any)._poll = undefined;
              } catch {}
            }, 10_000);
            ...
            (async () => {
              ...
              let payload =
                ...
                {};
              while (
                !(payload && (payload as any).niconama) &&
          ...
          cancel() {
            try {
              clearInterval((this as any)._keepalive);
            } catch {}
            try {
              clearInterval((this as any)._poll);
            } catch {}
            try {
              clearTimeout((this as any)._pollStop);
            } catch {}
          },
        });
