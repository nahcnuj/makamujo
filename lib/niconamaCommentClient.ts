  ...
  private async setupDirectWebSocketConnection(
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        this.sendDirectWebSocketMessage({
          type: "keepSeat",
          audienceToken: this.#directWebSocketAudienceToken,
        });
      };
        ...
        const attachListeners = (pageRef: any) => {
          ...
          pageRef.on("close", async () => {
            try {
              ...
              console.debug("[DEBUG] Playwright page closed", {
                url: pageUrlSafe,
              });
              if (this.#stopRequested) return;
              ...
              try {
                console.warn(
                  "[WARN] recreating Playwright context due to page close",
                  { url: watchUrl },
                );
                try {
                  await context.close();
      ...
      case "reconnect": {
        try {
          ...
          if (typeof newToken === "string" && newToken.length > 0) {
            ...
            console.info("[INFO] direct websocket requested reconnect", {
              wsUrl,
              waitTimeMs,
              newToken,
              reconnectUrl,
            ...
            console.info(
              "[INFO] direct websocket requested reconnect (token not provided explicitly)",
              { wsUrl, waitTimeMs },
            );
            if (!this.#stopRequested) {
              this.#directWebSocketReconnectTimer = globalThis.setTimeout(
