...
describe("Niconama comment helpers", () => {
  it("extracts trimmed text from AgentComment-like objects", () => {
    const item = {
      data: { comment: "  hello world  ", no: 1, userId: "user123" },
    };
  ...
  it("returns null for non-string or empty fields", () => {
    expect(
      getCommentTextFromAgentComment({ data: { comment: "" } }),
    ).toBeNull();
  ...
  it("filters comments without usable text", () => {
    ...
    expect(
      filtered.map((item) => getCommentTextFromAgentComment(item)),
  ...
  it("formats recent comment entries with numbers when available", () => {
    expect(
      formatAgentCommentEntry({ data: { comment: "hello", no: 123 } }),
    ).toBe("#123 hello");
    expect(formatAgentCommentEntry({ data: { comment: "world" } })).toBe(
      "world",
    );
    expect(
      formatAgentCommentEntry({ data: { comment: "(コメントあり)" } }),
    ).toBeNull();
  });
  ...
  it("normalizes comment text with a matching numbered prefix", () => {
    expect(
      getCommentTextFromAgentComment({
        data: { comment: "#2 こんにちは", no: 2 },
      }),
    ).toBe("こんにちは");
    expect(
      formatAgentCommentEntry({ data: { comment: "#2 こんにちは", no: 2 } }),
    ).toBe("#2 こんにちは");
  });

  it("does not duplicate comment numbers when text already begins with the same prefix", () => {
    expect(
      formatAgentCommentEntry({ data: { comment: "#123 hello", no: 123 } }),
    ).toBe("#123 hello");
    expect(
      formatAgentCommentEntry({ data: { comment: "#99 こんにちは", no: 99 } }),
    ).toBe("#99 こんにちは");
  });
});
