...
- ドメイン再設計・`MakaMujo` 分割・配信状態ペイロード変更では、観測可能な振る舞いを変えないこと。番組情報の変更は先に `architecture/program-info-reader.md` を読む。
- 番組情報（視聴者数 / コメント数 / ニコニコ広告ポイント / ギフトポイント）は **わんコメではなく、番組配信ページをブラウザで描画した画面**から読む。値は unama WebSocket 経由でしか画面に無いため HTML からの取得はできない。読むページは本番ではコードに固定（`DEFAULT_NICONAMA_WATCH_PAGE_URL`）。ブラウザ reader を使わないテストは `NICONAMA_WATCH_PAGE_DISABLED=1` を設定する（その場合 `POST /api/meta` の `niconama` へフォールバック）。
