...
- 番組情報（視聴者数 / コメント数 / ニコニコ広告ポイント / ギフトポイント）は **わんコメではなく、番組配信ページをブラウザで描画した画面**から読む。値は unama WebSocket 経由でしか画面に無いため HTML からの取得はできない。`NICONAMA_WATCH_PAGE_URL` が無ければ reader は起動せず、`POST /api/meta` の `niconama` へフォールバックする。
