  ...
  page.on('response', async (res) => {
    try {
      ...
      diagnostics.responses.push({
        ...
        status,
        headers,
        body: typeof body === 'string' ? (body.length > 10000 ? body.slice(0, 10000) + '...TRUNCATED' : body) : String(body),
      });
    } catch (e) {
      // ignore
    }
    ...
    for (let i = 0; i < frames.length; i++) {
      try {
        ...
        diagnostics.frames.push({
          url: frames[i].url(),
          index: i,
          snippet: typeof frameHtml === 'string' ? frameHtml.slice(0, 2000) : String(frameHtml),
        });
