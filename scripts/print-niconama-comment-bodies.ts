...
const DURATION_MS = process.env.NICONAMA_PRINT_DURATION_MS ? Number(process.env.NICONAMA_PRINT_DURATION_MS) : undefined;
...
async function main() {
  ...
  const client = createNiconamaCommentClient({ watchUrl: WATCH_URL, userDataDir: USER_DATA_DIR }, {
    onComments: (comments) => {
