...
if (!url) {
  console.error('Usage: bun ./scripts/captureReliveWs.js <wss-url> [audienceToken]');
  process.exit(1);
}
...
ws.addEventListener('open', () => {
  ...
  if (token) {
    ...
    const keep = () => {
      ...
      try { ws.send(JSON.stringify(msg)); console.log(new Date().toISOString(), 'sent keepSeat'); } catch (e) { console.error('send error', e); }
    };
    keep();
...
ws.addEventListener('message', (ev) => {
  ...
  try {
    ...
    } else if (data instanceof ArrayBuffer || (ArrayBuffer.isView && ArrayBuffer.isView(data))) {
