...
const ensureJapaneseFonts = () => {
  ...
  const fontCheckResult = spawnSync(
    "bash",
    ["-lc", "fc-list :lang=ja | grep -qi 'Noto Sans CJK'"],
    {
      stdio: "ignore",
    },
  );
  ...
  if (installResult.status !== 0) {
    throw new Error(
      "日本語フォントの導入に失敗しました。`sudo apt-get install -y fonts-noto-cjk` を実行してください。",
    );
  }
};
