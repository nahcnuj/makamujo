...
async function main() {
  ...
  if (page.status === 0) {
    console.error("fetch failed", page.text);
    process.exit(1);
  }
  ...
  if (site?.pollingApiBaseUrl)
    candidates.push(
      `${site.pollingApiBaseUrl}programs/${programId}/comments?limit=100`,
    );
  if (site?.frontendPublicApiUrl)
    candidates.push(
      `${site.frontendPublicApiUrl}programs/${programId}/comments?limit=100`,
    );
  if (site?.frontendPublicApiUrl)
    candidates.push(
      `${site.frontendPublicApiUrl}programs/${numeric}/comments?limit=100`,
    );
  if (relive?.apiBaseUrl)
    candidates.push(
      `${relive.apiBaseUrl}programs/${programId}/comments?limit=100`,
    );
  if (relive?.apiBaseUrl)
    candidates.push(
      `${relive.apiBaseUrl}programs/${numeric}/comments?limit=100`,
    );
  ...
  candidates.push(
    `https://papi.live.nicovideo.jp/programs/${numeric}/comments`,
  );
  candidates.push(
    `https://live2.nicovideo.jp/unama/api/v2/programs/${programId}/comments`,
  );
  candidates.push(
    `https://live2.nicovideo.jp/unama/api/v2/programs/${numeric}/comments`,
  );
