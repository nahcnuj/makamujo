...
| 統計行 | StatisticsRow | ページ内の 5 枠。HTML では全て `-`、JS 実行後に実値が入る |
| 視聴者数 | ViewerCount | 統計行の `watch-count-item` の表示値（`-` なら undefined） |
| コメント数 | PageCommentCount | 統計行の `comment-count-item` の表示値 |
| ニコニコ広告ポイント | NicoadPoint | 統計行の `nicoad-count-item` の表示値。**件数ではない** |
| ギフトポイント | GiftPoint | 統計行の `gift-count-item` の表示値。**件数ではない** |
...
| **Program Info Reader** | 配信ページの描画済み統計行 → `StreamData` | Playwright で読む。採取 30s。`NICONAMA_WATCH_PAGE_URL` 未設定なら起動しない |

**番組情報の唯一の供給元（確定）**: **Program Info Reader**（`composition/watchPageBrowserReader.ts` + `composition/watchPageBrowserSession.ts` + `lib/domain/broadcasting/watchPage{Statistics,Program}.ts` + `lib/application/ProgramInfoAssembler.ts`）。`POST /api/meta` の `niconama` / `points` は **公開ペイロードの `niconama` と `commentCount` には効かない**（`replyTargetComment` などのみ従来どおり効く）。
...
```mermaid
flowchart LR
  Browser["ブラウザで配信ページを描画"] --> Read["統計行の表示テキストを読む"]
  Read --> Parse["parseDisplayedStatistics"]
...
| `programInfo` | host `watchPageProgramInfo`（配信ページの描画済み統計行 → `StreamData` → 正規化）。**未取得なら `undefined`** |
...
### Program Info Reader（番組配信ページの描画済み画面の読み取り）

wancomme の `POST /api/meta` が送ってきた `total` / `points` に代えて、**配信ページをブラウザで描画し、画面に表示されている値を読む**。

**なぜレンダリングが必要か（確定）**:

- 配信ページの統計行は 5 枠（視聴者数 / コメント数 / タイムシフト予約数 / ニコニコ広告ポイント / ギフトポイント）を持ち、**HTML では全て `-` プレースホルダ**である（`data-blank="true"`）。
- 実値は unama WebSocket の `dwango.nicolive.chat.data.Statistics`（`viewers` / `comments` / `ad_points` / `gift_points`）をページの JS が受けて埋める。HTML には `adPoint` / `giftPoint` が **0 箇所**しか存在しない。
- HTTP で完結する統計 API は存在しない（`/api/live/{lv}/program` 等は 404。`pollingApiBaseUrl` = `papi.live.nicovideo.jp` は `/api/relive/notifybox.*` 専用）。
...
| ニコニコ広告 | `nicoad-count-item` | **ニコニコ広告ポイント** |
| ギフト | `gift-count-item` | **ギフトポイント** |
...
**`undefined` の意味**: ページが `-` を表示している（JS がまだ埋めていない / 値が無い）とき `undefined` を返し、`StreamMeta.total` の該当キーごと落とす。コンソールは `formatMetricValue(undefined) === "-"` でページと同じ `-` を出す。

**モジュール**:

| モジュール | 層 | 役割 |
|------------|----|------|
| `lib/domain/broadcasting/watchPageStatistics.ts` | domain（純関数） | 表示テキスト → 数値（`-` / `,` / `万` / `億` 対応） |
| `lib/domain/broadcasting/watchPageProgram.ts` | domain（純関数） | `embedded-data` → `WatchPageProgram` / `undefined` |
...
| `composition/watchPageBrowserSession.ts` | composition | Playwright + stealth 実装。**必要なときだけ動的 import** |
...
| `NICONAMA_WATCH_PAGE_READ_INTERVAL_MS` | `30000` | 採取周期。ページ側は 30〜60 秒粒度なのでこれより短くしても無駄 |

**ブラウザ操作の契約**:

- ページは**一度だけ開き、以降は開いたまま**統計行の表示テキストだけを読み直す（ページ自身が WebSocket で更新するため）。30 秒ごとに再読み込みはしない。
- 読み取りに失敗したら `onError` に通知し、セッションを破棄する。**`programInfo` は直前の値を保つ**（勝手にはオフラインにしない）。次の採取で作り直す。
- **ページが読めていない（空 / エラーページ / JS 実行前）ときも同じ**：`read()` が throw し、`onSnapshot` には到達しない。空の読み取りを「配信終了」と誤解して番組情報を飛ばさないため。
- 配信ページが 200 で応答し、`status` が `ON_AIR` 以外なら **オフライン**として扱う（実ページでは配信していないユーザーでも直近の番組が載るため）。
- したがって「配信ページが 404 / 到達不能」の場合はオフラインにならず**直前値を維持**する。これは意図した trade-off（空読み取りで正しい番組情報を飛ばさないことを優先）。
